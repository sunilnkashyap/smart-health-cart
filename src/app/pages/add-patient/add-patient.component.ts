...
export class AddPatientComponent extends BasePage implements OnInit {
  ...
  getOldData() {
    this.getFireData('users/'+this.editId).subscribe((userData) => {
      console.log(userData, this.editId)
      userData.password = '';
      delete userData.uid;
    ...
    if(this.form.valid) {
      ...
      if(this.editId && this.editId != '') {
        ...
        this.setFireData(path, formData).then((setFireDataResponse) => {
          ...
          this.notification('Patient edited successfully!');
          this.navigateTo('dashboard/patients')
        }).catch((setError) => {
        ...
        this.fireSignUp(formData.email, formData.password).then((data: any) => {
          ...
          this.setFireData(path, formData).then((setFireDataResponse) => {
            console.log(setFireDataResponse);
            this.notification('Patients added successfully!');
            this.navigateTo('dashboard/patients')
          }).catch((setError) => {
            console.log(setError);
            this.showError = false;
